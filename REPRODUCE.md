...
- **Defensible framing**: **do not claim criticality-conditioning improves compliance prediction, on either discrimination or calibration, at either scope.** The rigorous finding is a null result across the board once corrected for the 9 simultaneous tests. What *is* still worth reporting honestly: the point estimates are directionally consistent with a small positive effect in most comparisons (see raw point_diff columns), and oracle's point estimate is consistently above conditioned/uniform — but neither claim currently clears a corrected significance bar. Report this as "no statistically robust evidence of a conditioning effect at current N" rather than a positive finding.
...
- **Tuning caveat:** the frozen config (`LATENT_TO_LABEL_NOISE`, `LATENT_TO_FEATURE_NOISE`, `CRIT_PROB_SHARPEN`) was searched to produce a conditioning effect (see "Tuned configuration" above) — disclose this search process. Combined with the corrected-null result above, this is an important joint caveat: the DGP was searched specifically to find a regime with this effect, and even in that searched-for regime, the effect doesn't survive rigorous testing.
